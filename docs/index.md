# Documentation Index

Use this index to navigate the blueprint by lifecycle phase.

## Start Here

- [Repository Review](review/repo-review.md)
- [GitHub Publishing Checklist](review/github-publishing-checklist.md)
- [Repository Separation Model](architecture/repo-separation-model.md)
- [Recommended Repositories](architecture/recommended-repos.md)

## Architecture

- [Multi-Repo Model Diagram](architecture/diagrams/multi-repo-model.md)
- [CI/CD Boundaries Diagram](architecture/diagrams/ci-cd-boundaries.md)
- [GitOps Promotion Flow Diagram](architecture/diagrams/gitops-promotion-flow.md)
- [Client Onboarding Flow Diagram](architecture/diagrams/client-onboarding-flow.md)
- [Platform vs App Responsibility Diagram](architecture/diagrams/platform-vs-app-responsibility.md)
- [Architecture Decision Records](architecture/decision-records/)

## Security And Public Safety

- [Secret Management Policy](security/secret-management-policy.md)
- [Public/Private Boundary](security/public-private-boundary.md)
- [Repository Sanitization Checklist](security/repository-sanitization-checklist.md)
- [Security Gates](quality-gates/security-gates.md)

## Delivery And GitOps

- [Application CI Template](ci-cd/app-ci-template.md)
- [Deployment CD Template](ci-cd/deployment-cd-template.md)
- [Rollback Strategy](ci-cd/rollback-strategy.md)
- [Image Promotion Model](gitops/image-promotion-model.md)
- [Argo CD Application Patterns](gitops/argocd-application-patterns.md)
- [Environment Promotion](gitops/environment-promotion.md)

## Operations

- [Operating Model](operations/operating-model.md)
- [Support Boundaries](operations/support-boundaries.md)
- [Incident Response Overview](operations/incident-response-overview.md)
- [Edge Routing Model](operations/edge-routing-model.md)
- [Release Readiness Checklist](quality-gates/release-readiness-checklist.md)

## Onboarding And Standards

- [Client Organization Bootstrap](client-onboarding/client-org-bootstrap.md)
- [New Client Repository Split](client-onboarding/new-client-repo-split.md)
- [New Project Checklist](checklists/new-project-checklist.md)
- [Naming Conventions](naming/naming-conventions.md)
- [Access Control Model](access-control/repo-access-model.md)

## Tooling

- [Official Links](tooling/official-links.md)
- [Recommended Tools](tooling/recommended-tools.md)
- [Local Development Tooling](tooling/local-dev-tooling.md)
